...
let endpoint=f.url.startsWith(PROXY+"/news?q=")?f.url:PROXY+"/rss?url="+encodeURIComponent(f.url);
let r;
...
try{let a=await getFeed(f),m=new Map(S.articles.map(x=>[x.id,x]));a.forEach(x=>m.set(x.id,x));S.articles=[...m.values()].slice(0,4000);save();render();if(show)toast("Feed refreshed")}catch(e){toast(e.message)}}
...
if(p==="home")render();if(p==="feedspage")renderFeeds();if(p==="bundlespage")renderBundles();
if(p==="filterspage"){$("white").value=S.white.join(", ");$("black").value=S.black.join(", ");$("dedupe").checked=S.dedupe;$("unread").checked=S.unread}
}
function add(){
let s=$("fBundle");s.innerHTML='<option value="">No bundle</option>'+S.bundles.map(b=>`<option value="${b.id}">${esc(b.name)}</option>`).join("");
$("feedForm").reset();$("testMsg").textContent="";if(typeof feedDlg.showModal==="function")feedDlg.showModal();else feedDlg.setAttribute("open","")
}
$("newFeed").onclick=$("newFeed2").onclick=add;
$("testFeed").onclick=async()=>{let u=$("url").value.trim()||keywordURL($("keyword").value.trim());if(!u)return $("testMsg").textContent="Enter a keyword or RSS URL";try{let a=await getFeed({id:"test",url:u});$("testMsg").textContent="✓ Feed works — "+a.length+" stories found"}catch(e){$("testMsg").textContent="✕ "+e.message}};
$("feedForm").onsubmit=async e=>{e.preventDefault();let k=$("keyword").value.trim(),u=$("url").value.trim()||keywordURL(k);if(!k&&!$("url").value.trim())return toast("Enter keyword or RSS URL");if(S.feeds.some(f=>f.url===u))return toast("This feed already exists");let f={id:crypto.randomUUID(),name:$("fname").value.trim()||k||"RSS Feed",keyword:k,url:u};S.feeds.push(f);let b=S.bundles.find(x=>x.id===$("fBundle").value);if(b)b.feeds.push(f.id);save();feedDlg.close();renderFeeds();await refreshFeed(f,false);toast("Feed saved")};
$("newBundle").onclick=()=>$("bundleDlg").showModal();
$("bundleForm").onsubmit=e=>{e.preventDefault();S.bundles.push({id:crypto.randomUUID(),name:$("bname").value.trim(),feeds:[]});save();bundleDlg.close();renderBundles();toast("Bundle created")};
$("refresh").onclick=async()=>{for(const f of S.feeds)await refreshFeed(f,false);toast("All feeds refreshed")};
$("q").oninput=render;
$("saveWhite").onclick=()=>{S.white=$("white").value.split(/[,\n]/).map(x=>x.trim()).filter(Boolean);save();render();toast("Whitelist saved")};
$("saveBlack").onclick=()=>{S.black=$("black").value.split(/[,\n]/).map(x=>x.trim()).filter(Boolean);save();render();toast("Blacklist saved")};
$("dedupe").onchange=e=>{S.dedupe=e.target.checked;save();render()};
$("unread").onchange=e=>{S.unread=e.target.checked;save();render()};
$("export").onclick=()=>{let a=document.createElement("a");a.href=URL.createObjectURL(new Blob([JSON.stringify(S,null,2)],{type:"application/json"}));a.download="marketfeed-backup.json";a.click()};
