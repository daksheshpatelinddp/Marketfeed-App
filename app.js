...
function renderFeeds() {
  $("feeds").innerHTML = S.feeds.length
    ? S.feeds.map(f => `
      <div class="card feed-row" data-select-feed="${esc(f.id)}">
        <div class="feed-name">
          <b>${esc(f.name)}</b>
        ...
        <div class="actions">
          <button type="button" class="small"
            data-add-bundle="${esc(f.id)}">Add to bundle</button>

          <button type="button" class="small"
            data-filter-feed="${esc(f.id)}">Filter</button>

          <button type="button" class="small"
            data-rf="${esc(f.id)}">Refresh</button>

          <button type="button" class="small"
            data-df="${esc(f.id)}">Delete</button>
        </div>
      </div>
    `).join("")
    : `<div class="empty">No feeds yet.</div>`;

  // Selecting the feed opens News/Home for that feed only
  document.querySelectorAll("[data-select-feed]").forEach(row => {
    row.onclick = e => {
      if (e.target.closest("button")) return;

      activeFeed = row.dataset.selectFeed;
      bundle = "";
  ...
  document.querySelectorAll("[data-rf]").forEach(b => {
    b.onclick = e => {
      e.stopPropagation();
      refreshFeed(
        S.feeds.find(f => f.id === b.dataset.rf)
      );
    };
  });
  ...
  document.querySelectorAll("[data-df]").forEach(b => {
    b.onclick = e => {
      ...
      S.bundles.forEach(bu => {
        bu.feeds = bu.feeds.filter(
          id => id !== b.dataset.df
        );
      });
